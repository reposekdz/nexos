...
console.log('🚀 Nexos Platform - Complete All Routes Generation\n');

const routesDir = path.join(__dirname, 'backend', 'routes');
let stats = { created: 0, existing: 0 };

const createRoute = (filename, content) => {
...
// Execute batch route creation
try {
  const routes = require('./route-definitions');
  
  for (const [filename, content] of Object.entries(routes)) {
    createRoute(filename, content);
  }
  
  console.log(`\n✅ Complete! Created: ${stats.created}, Existing: ${stats.existing}`);
} catch (error) {
  console.error('Error:', error.message);
  
  // Fallback: create routes directly
  console.log('\n📝 Creating routes directly...\n');
  
  // Phone Auth Route
  createRoute('phone-auth.js', `const express = require('express');
  ...
  if (process.env.TWILIO_ACCOUNT_SID) {
    try {
      ...
      await twilio.messages.create({
        body: \`Your Nexos code: \${code}. Valid 10 mins.\`,
  ...
  try {
    ...
    if (!user) {
      ...
      user = await User.create({
        username,
        email: \`\${username}@nexos-phone.temp\`,
    ...
    const token = jwt.sign({ id: user._id, phoneNumber: phoneNormalized }, process.env.JWT_SECRET, { expiresIn: '7d' });
...
router.post('/contacts/sync', auth, async (req, res) => {
  try {
    ...
    for (const contact of contacts) {
      ...
      let existingContact = await Contact.findOne({ owner: req.user.id, phoneNormalized });

      if (existingContact) {
        existingContact.name = contact.name || existingContact.name;
        existingContact.linkedUser = linkedUser?._id;
      ...
      if (linkedUser) matched.push({ contact: phoneNormalized, user: linkedUser._id });
    }

    res.json({ message: 'Contacts synced', synced: synced.length, matched: matched.length, contacts: synced });
  } catch (error) {
...
`);
  ...
  // Calls Enhanced Route
  createRoute('calls-enhanced.js', `const express = require('express');
...
router.post('/initiate', auth, async (req, res) => {
  try {
    ...
    if (!recipientId || !callType) return res.status(400).json({ error: 'Recipient and call type required' });
    ...
    const call = await Call.create({
      ...
      startTime: new Date(),
      deviceInfo: {
        caller: { type: req.body.deviceType || 'unknown', os: req.body.os, browser: req.body.browser }
      },
      webrtc: {
        sessionId: \`session_\${Date.now()}_\${Math.random().toString(36).substr(2, 9)}\`,
        signaling: []
      },
      encryption: { enabled: true, algorithm: 'AES-256-GCM', keyExchange: 'DTLS-SRTP' }
    });
...
router.post('/:id/accept', auth, async (req, res) => {
  try {
    ...
    if (call.recipient.toString() !== req.user.id) return res.status(403).json({ error: 'Not authorized' });

    call.status = 'connected';
    call.deviceInfo.recipient = { type: req.body.deviceType || 'unknown', os: req.body.os, browser: req.body.browser };
    await call.save();

    const io = req.app.get('io');
    if (io) {
      io.to(call.caller.toString()).emit('call-accepted', { callId: call._id, sessionId: call.webrtc.sessionId });
    }
...
router.post('/:id/decline', auth, async (req, res) => {
  try {
    ...
    const io = req.app.get('io');
    if (io) io.to(call.caller.toString()).emit('call-declined', { callId: call._id });
...
router.post('/:id/end', auth, async (req, res) => {
  try {
    ...
    if (req.body.quality) call.quality = req.body.quality;
    await call.save();
...
router.post('/:id/signal', auth, async (req, res) => {
  try {
    ...
    call.webrtc.signaling.push({ type, data, timestamp: new Date() });
    await call.save();
...
router.post('/:id/screen-share/start', auth, async (req, res) => {
  try {
    ...
    call.features.screenSharing = { enabled: true, startedAt: new Date() };
    await call.save();
...
router.post('/:id/reaction', auth, async (req, res) => {
  try {
    ...
    call.features.reactions.push({ user: req.user.id, emoji, timestamp: new Date() });
    await call.save();
...
`);
  ...
  console.log(`\n✅ Phase 1 Complete! Created: ${stats.created}, Existing: ${stats.existing}`);
}
